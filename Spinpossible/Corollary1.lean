...
lemma spin_eq_swap_of_adj {p1 p2 : Point m n} {r : Rectangle m n} (h : p1.IsAdjacent p2)
    (hr : p1 = r.topLeft ∧ p2 = r.bottomRight) :
  ...
  · rw [Equiv.swap_apply_of_ne_of_ne]
    · by_contra k
      ...
      exact h7 r.corners_inside.1
    · by_contra k
      simp only [k, to2d_to1d_inverse] at h7
      exact h7 r.corners_inside.2
...
lemma spin_s11_s12_closure (m n : PNat) : Subgroup.closure ((mySet m n).toSet) = ⊤ := by
  ...
  let ⟨l, hl, hl2⟩ : ∃ l : List (Equiv.Perm (Fin (m * n))), (∀ x ∈ l, x.IsAdjacentSwap) ∧ l.prod = s.α := by
    ...
    have perm_length : r.toSpin.α.support.toList.length = 2 :=
      Equiv.Perm.card_support_eq_two.mpr sw ▸ hr2 ▸ Finset.length_toList _
    have adj := Rectangle.swap_iff.mp hr1
    have r_corners : r.topLeft ≠ r.bottomRight := by
    ...
    have top_in : to1d r.topLeft ∈ r.toSpin.α.support.toList := by simp [this]
    have bot_in : to1d r.bottomRight ∈ r.toSpin.α.support.toList := by simp [this]
    ...
    · simp only [← hr2, ← htop, to2d_to1d_inverse]
