...
lemma foldl_apply_eq_scanl_last {α : Type*}(l : List (Perm α)) (x : α) :
...
lemma foldl_eq_prod_reverse {α : Type*} (l : List (Equiv.Perm α)) (x : α) :
...
lemma graph_connected.aux1 [DecidableEq α]
  ...
  rw [← foldl_apply_eq_scanl_last l x, foldl_eq_prod_reverse l x, h_prod_reverse, h]
  simp
...
lemma graph_connected [DecidableEq α] [Nonempty α] (E : Set (Equiv.Perm α))
  ...
  apply Connected.mk
  set G := SimpleGraph.fromRel (λ x y => swap x y ∈ E)
  ...
  have ⟨l, hlE, hl_prod⟩ : ∃ l : List (Equiv.Perm α), (∀ τ ∈ l, τ ∈ E) ∧ l.prod = swap x y := by
...
theorem transpositions_generate_symm_group_iff_connected_graph
  ...
  · intro hG_connected
    ...
    intro σ
    induction' σ using Equiv.Perm.swap_induction_on with τ a b hab hτ
    ...
    · clear hab
      ...
      · by_cases h_eq : x = z
        ...
        have swap_xz_eq : (swap y z) * (swap x y) * (swap y z) = swap z x :=
          Equiv.swap_mul_swap_mul_swap (isSwap_swap_ne (hE _ swap_xy_in_E)) h_eq
